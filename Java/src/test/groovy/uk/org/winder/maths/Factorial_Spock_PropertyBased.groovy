...
class Factorial_Spock_PropertyBased extends Specification {
  ...
  def '#name(#n) obeys the factorial relation for positive integer arguments'() {
    expect: f.call(n + 1) == (n + 1) * f.call(n)
    where: [f, name, n] << algorithms.collectMany{a -> Gen.integer(0, 1000).take(20).collect{[*a, it]}}
  }
  ...
  def '#name(#n) throws an exception due to negative integer argument'() {
    ...
    where: [f, name, n] << algorithms.collectMany{a -> Gen.integer(-1000, -1).take(10).collect{[*a, it]}}
  }
  ...
  def '#name(#n) throws an exception due to double argument'() {
    ...
    where: [f, name, n] << algorithms.collectMany{a -> Gen.integer(-1000, 1000).take(10).collect{[*a, it + 0.5]}}
  }
