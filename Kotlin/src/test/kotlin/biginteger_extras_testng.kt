...
class BigInteger_Extras_TestNG {
  ...
  fun value_is_in_a_range_it_is_supposed_to_be() {
    assertTrue(5.bigint in 0.bigint .. 10.bigint)
  }

  Test
  fun value_is_not_in_a_range_it_is_not_supposed_to_be_in() {
    assertFalse(-5.bigint in 0.bigint .. 10.bigint)
  }

  Test
  fun upward_range_generates_the_expected_values() {
    assertEquals((0.bigint .. 5.bigint).toList(), listOf(0, 1, 2, 3, 4, 5).map{it.bigint})
  }

  Test
  fun upward_stridded_range_generates_the_expected_values() {
    assertEquals((0.bigint .. 5.bigint step 2).toList(), listOf(0, 2, 4).map{it.bigint})
  }
