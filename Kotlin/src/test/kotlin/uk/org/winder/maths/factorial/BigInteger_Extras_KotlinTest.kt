...
class BigInteger_Extras_KotlinTest : StringSpec({
    ...
    "a value is in a range it is supposed to be in" {
        5.bigint in 0.bigint rangeTo 10.bigint
    }
    ...
    "a positive value above the positive value range is not in the range" {
        15.bigint !in 0.bigint rangeTo 10.bigint
    }

    "an upward range generates the expected values" {
        (0.bigint rangeTo 5.bigint).toList() == listOf(0, 1, 2, 3, 4, 5).map { it.bigint }
    }

    "an upward strided range generates the expected values" {
        (0.bigint rangeTo 5.bigint step 2).toList() == listOf(0, 2, 4).map { it.bigint }
    }
